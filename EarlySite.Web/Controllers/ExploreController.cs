namespace EarlySite.Web.Controllers
{
    ...
    public class ExploreController : BaseController
    {
        ...
        public ActionResult RecipeView(int recipeId)
        {
            ...
            //获取单品列表
            Result<IList<Dish>> dishlist = ServiceObjectContainer.Get<IDishService>().GetCollectDishList(recipeId);
            ViewBag.DishList = dishlist.Data;
            //是否是自己的食谱
            ViewBag.IsSelf = recipe.Data.Phone == CurrentAccount.Phone;
            //获取相似食谱
            Result<IList<Recipes>> recipes = ServiceObjectContainer.Get<IRecipesService>().GetSomeRecpiesByRecipeId(recipeId, 8);
